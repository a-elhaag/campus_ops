  ...
  async function updateStatus(id: string, newStatus: TaskStatus) {
    try {
      ...
      if (res.ok) {
        ...
        toast.success(
          `Moved to ${statusLabels[newStatus as keyof typeof statusLabels]}`,
        );
        router.refresh();
      } else {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
          Active Operations{" "}
          <Badge className="rounded-full bg-[#4A6E91]/10 text-[#4A6E91] border-none">
            {tasks.length}
          </Badge>
        </h3>
        ...
        {tasks.length === 0 && (
          <div className="p-12 text-center rounded-[2rem] neo-pressed border-2 border-dashed border-gray-200">
            <p className="text-gray-400 italic">
              No tasks active in this sector.
            </p>
          </div>
        )}
          ...
          {tasks.map((task) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-2 rounded-xl neo-pressed ${task.status === "done" ? "text-[#4A6E91]" : "text-gray-400"}`}
                >
                  {task.status === "done" ? (
                ...
                <div>
                  <p
                    className={`font-bold transition-all ${task.status === "done" ? "line-through text-gray-400 font-medium" : "text-gray-800"}`}
                  >
                    {task.title}
                  </p>
                  <div className="flex gap-2 mt-1">
                    <Badge
                      variant="outline"
                      className="text-[10px] font-bold uppercase border-gray-200 text-gray-500"
                    >
                      {task.role}
                    </Badge>
                  </div>
                ...
                <div className="flex items-center gap-2 p-1 rounded-xl neo-pressed bg-white/50">
                  {statuses.map((s) => (
                    <button
                      key={s.value}
                      onClick={() =>
                        updateStatus(task.id, s.value as TaskStatus)
                      }
                      title={s.label}
            ...
            >
              ...
              <form onSubmit={handleCreate} className="space-y-6">
                <div className="space-y-2">
                  <Label className="ml-2 text-xs font-bold uppercase tracking-widest text-gray-400">
                    Operation Name
                  </Label>
                  <Input
