...
export default function RsvpTable({ slug, initialRsvps }: { slug: string, initialRsvps: RSVP[] }) {
    ...
    async function toggleCheckIn(id: string, checked_in: boolean) {
        // Optimistic update
        setRsvps(prev => prev.map(r => r.id === id ? { ...r, checked_in } : r));
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Revert on failure
                setRsvps(prev => prev.map(r => r.id === id ? { ...r, checked_in: !checked_in } : r));
            }
        } catch (e) {
            // Revert on error
            setRsvps(prev => prev.map(r => r.id === id ? { ...r, checked_in: !checked_in } : r));
    ...
    function exportToExcel() {
        ...
        link.setAttribute("download", `rsvps-${slug}-${format(new Date(), 'yyyyMMdd-HHmmss')}.csv`);
